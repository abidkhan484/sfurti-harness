# 33 — Route only critical editorial cases to human attention

Status: ready-for-agent
Implementation: not started
Dependencies: [24](24-final-content-review.md), [31](31-refresh-withdrawal.md)
Execution gate: dependencies must be implemented and their acceptance checks passing.
Spec references: S4, S8
...
Human attention is reserved for concrete critical exceptions, with automatic routine handling.

## Files and boundaries

- src/research/escalation.ts
- src/notifications.ts
- test/research-escalation.test.ts
...
1. Implement deterministic escalation policy around reviewed severity and actual affected state. Critical categories follow S4 and require reason/evidence.
2. Medium/low failures defer/reject automatically and appear in passive status/digest; retry exhaustion alone never escalates.
3. Create deduplicated critical notifications with affected claims/posts, containment status and concrete required action. Preserve existing delivery outbox retry behavior.
4. Keep operational authentication/configuration requests distinct from editorial human review and avoid alert flooding from repeated polls.
5. Resolve a critical case only when containment/review is recorded, not merely when a notification was sent.

## Acceptance checks

- Three ordinary rejected drafts cause no human-review event.
- Consequential already-published error emits one critical alert with remote ID.
- Repeated failed delivery retries same notification; unknown cancellation remains open.

## Verification

Run from /infinity/codes/own/sfurti-harness.

    node --test test/research-escalation.test.ts
