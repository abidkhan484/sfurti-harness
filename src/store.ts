...
export class Store {
  ...
  all<T = RecordData>(collection: string): T[] {
    return this.db.prepare(`SELECT data FROM ${this.table(collection)} ORDER BY rowid`).all()
      .map(row => JSON.parse(String(row.data)));
  }
  put<T extends {id: string}>(collection: string, value: T): T {
    if (!value.id) throw new Error('Record ID is required');
    this.db.prepare(`INSERT INTO ${this.table(collection)}(id,data) VALUES (?,?) ON CONFLICT(id) DO UPDATE SET data=excluded.data`)
  ...
  transaction<T>(body: () => T): T {
    ...
    try {
      const result = body();
      if (result instanceof Promise) throw new Error('Transactions cannot contain asynchronous work');
      this.db.exec('COMMIT');
      return result;
    } catch (error) {
