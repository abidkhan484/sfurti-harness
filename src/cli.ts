...
try {
  ...
  if (!env.SFURTI_CONFIG && existsSync("./config/harness.json"))
    env.SFURTI_CONFIG = "./config/harness.json";
    ...
    if (name === "start") {
      ...
      if (name === "command") command = JSON.parse(values.json ?? readFileSync(0, "utf8"));
        ...
        command = {
          type: "request",
          topic: values.topic,
          ageSegment: values.age,
          videos: Number(values.videos ?? 0),
          images: Number(values.images ?? 0),
          texts: Number(values.texts ?? 0),
          schedule: values.schedule === true,
          requestId: values["request-id"],
        };
      else if (name === "source" && positionals[1] === "register")
        command = {
          type: "register-source",
          sourceId: values["youtube-id"],
          language: values.language,
          filePath: values.file,
          permission: JSON.parse(readFileSync(values["permission-file"] ?? "", "utf8")),
        };
      else if (name === "sources" && positionals[1] === "pending")
        command = { type: "pending-sources" };
      else if (name === "retry")
        command = { type: "retry-artifact", artifactId: positionals[1], reason: values.reason };
      else if (name === "pause" || name === "resume")
        command = { type: "pause", paused: name === "pause" };
      else
        command = {
          type: name,
