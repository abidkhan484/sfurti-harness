...
export async function tick(ctx: Context, command: Command, execute: Execute) {
  ...
  try {
    ...
    for (const job of pending) {
      ...
      for (const kind of Object.keys(kinds) as Array<keyof typeof kinds>) {
        for (let index = 0; index < job.counts[kind]; index++) {
          ...
          attempted++;
          const artifact: any = await production(ctx, {
            ...job.command,
            type: "produce",
            origin: "custom",
            kind,
    ...
    const dailyDue = ctx.store.transaction(() => {
      if (
        command.daily !== true &&
        (!ctx.config.daily.productionTimes.includes(time) || ctx.store.get("meta", opportunityId))
      )
        return false;
      ctx.store.put("meta", {
        id: opportunityId,
    ...
    const snapshot = dailyDue
      ? await execute({ type: "daily-snapshot" })
      : ctx.store.get("plans", date);
    ...
    for (const day of coverage.days) {
      ...
      for (const kind of Object.keys(kinds) as Array<keyof typeof kinds>) {
        ...
        for (let slot = 0; slot < (day.missing?.[kind] ?? 0); slot++) {
                  ...
                  !allSegments.some(
                    (segment) =>
                      ...
                      s.metadata.segments.some(
                        (s: any) => s.startMs < segment.endMs && segment.startMs < s.endMs
                      )
                  )
              );
            source = eligible();
          ...
          attempted++;
          const artifact: any = await production(
    ...
    const digest = createHash("sha256")
      .update(
        JSON.stringify({
          ...
          missing: coverage.days.map((d: any) => [d.date, d.missing]),
