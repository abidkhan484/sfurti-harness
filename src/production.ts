...
import { fileIntegrity as validFile, assertSourcePermission as assertPermission, reviewCriteria as criteria, validReview } from './domain.ts';
...
async function inspect(ctx: Context, artifact: Artifact, version: any, source: any, signal: AbortSignal) {
  const findings: any[] = [];
  let evidence: any = {};
  try {
    ...
    const inspected = await ctx.adapters.media.inspect({filePath:version.filePath, kind:artifact.kind, source, segments:artifact.segments, signal});
    ...
    if (artifact.kind === 'text') {
      ...
      for (const frame of evidence.frames) {
        if (!frame || typeof frame !== 'object' || !Number.isFinite(frame.timeMs) || frame.timeMs < 0 || frame.timeMs > inspected.durationSeconds * 1000) throw new Error('Every video evidence frame requires a valid output timestamp');
    ...
    signal.throwIfAborted();
    if (['RATE_LIMIT','QUOTA_EXCEEDED','rate_limit','rate-limit','quota'].includes(error.code ?? error.kind)) throw error;
    findings.push(problem(version.number, 'usability', String(error), 'Provide a valid Bangla artifact and complete independent inspection evidence')); }
  if (ctx.adapters.reviewer === ctx.adapters.editor || (ctx.adapters.editor?.executionIdentity && ctx.adapters.editor.executionIdentity === ctx.adapters.reviewer?.executionIdentity) || !ctx.adapters.reviewer?.review) findings.push(problem(version.number,'independence','No independent reviewer','Use a separate reviewer adapter'));
  if (findings.length) return { passed:false, findings, evidence };
  signal.throwIfAborted();
  const review = await ctx.adapters.reviewer.review({ artifact, version, evidence, mission:ctx.mission, source, signal });
  signal.throwIfAborted();
  if (version.provider?.threadId && version.provider.threadId === review?.provider?.threadId) findings.push(problem(version.number,'independence','Editor and reviewer reused the same provider thread','Review in a fresh independent provider thread'));
  const failed = criteria.filter(key => review?.criteria?.[key] !== true);
  const malformed = !validReview(review) || review.findings.some(f => f.version !== version.number);
  ...
  return { ...review, passed:review?.passed === true && !failed.length && !malformed && !review.findings.length && !findings.length, findings:[...(Array.isArray(review?.findings) ? review.findings : []), ...findings], evidence };
}
...
export async function production(ctx: Context, command: Command): Promise<unknown> {
  ...
  if (command.type === 'discover') {
    ...
    const claim = store.transaction(() => {
      ...
      if (previous && previous.topic !== topic) throw new Error('Discovery request ID is already owned by a different topic');
      if (previous?.result) return {result:previous.result};
      if (previous?.status === 'started' && Date.parse(previous.leaseUntil) > ctx.now().getTime()) return {result:{status:'running',taskId}};
      if (!claimWorkload(ctx, taskId)) return {result:{status:'deferred',reason:'workload-limit'}};
      const batchId = previous?.batchId ?? ctx.id();
      store.put('meta',{id:taskId,batchId,topic,status:'started',leaseOwner:owner,leaseUntil:new Date(ctx.now().getTime()+ctx.config.limits.leaseMs).toISOString()});
      return {batchId};
    });
    ...
    let result: any;
    try {
      result = await ctx.adapters.discovery.discover({topic, mission:ctx.mission, language:'bn', untrustedSourceContent:true,idempotencyKey:taskId});
    } catch (error) {
      if (store.get<RecordData>('meta',taskId)?.leaseOwner === owner) store.put('meta',{id:taskId,batchId,topic,status:'failed',lastError:String(error)});
      throw error;
    }
    if (store.get<RecordData>('meta',taskId)?.leaseOwner !== owner) throw new Error('Discovery ownership lost');
    ...
    const keywords = result.keywords.map((keyword:any) => {
      ...
      return {...keyword,externalId:keyword.id,id:`${batchId}:${keyword.id ?? ctx.id()}`,batchId,topic,missionVersion:ctx.mission.version};
    });
    const sources = result.sources.map((source:any) => {
      ...
      if (qualification && ['relevant','credible','actualContentReviewed','locallyRelevant'].every(key=>qualification[key] === true)) {
        try {
          const integrity = validFile(qualification.evidencePath);
          metadata.segments = intervals(metadata.segments);
          metadata.qualification = {...qualification,integrity,missionVersion:ctx.mission.version};
          metadata.qualified = true;
        } catch { metadata.qualified = false; }
      }
      return {...existing,id:source.id,title:source.title,language:source.language,metadata,status:existing?.status ?? 'pending',discoveredAt:existing?.discoveredAt ?? ctx.now().toISOString()};
    });
    store.transaction(() => {
      store.put('keyword_batches',{id:batchId!,topic,mission:ctx.mission,createdAt:ctx.now().toISOString()});
      for (const keyword of keywords) store.put('keywords',keyword);
      for (const source of sources) store.put('sources',source);
      for (const match of result.matches ?? []) {
        const keyword = keywords.find((k:any) => k.externalId === match.keywordId || k.id === match.keywordId);
        if (!keyword || !sources.some((s:any) => s.id === match.sourceId)) throw new Error('Discovery match references unknown keyword or source');
        store.put('matches',{...match,keywordId:keyword.id,id:`${batchId}:${keyword.id}:${match.sourceId}`,batchId});
      }
    });
  ...
  if (command.type === 'register-source') {
    ...
    const source = {...existingSource,id:command.sourceId,language:command.language ?? existingSource?.language,filePath:command.filePath,permission:command.permission,status:'cleared'};
    ...
    const lineageDirectory = join(ctx.config.storage.mediaDirectory,'sources',createHash('sha256').update(source.id).digest('hex'));
    mkdirSync(lineageDirectory,{recursive:true});
    const retainedSource = join(lineageDirectory,`${sourceIntegrity.sha256}${extname(source.filePath) || '.bin'}`);
    const retainedPermission = join(lineageDirectory,`permission-${permissionIntegrity.sha256}${extname(source.permission.evidencePath) || '.txt'}`);
    if (source.filePath !== retainedSource) copyFileSync(source.filePath,retainedSource);
    if (source.permission.evidencePath !== retainedPermission) copyFileSync(source.permission.evidencePath,retainedPermission);
    Object.assign(source,{filePath:retainedSource,integrity:sourceIntegrity,permission:{...source.permission,evidencePath:retainedPermission,integrity:permissionIntegrity}});
    store.put('sources',source);
    store.put('permissions',{id:ctx.id(),sourceId:source.id,...source.permission,registeredAt:ctx.now().toISOString()});
    return source;
  }
  ...
  if (command.type === 'retry-artifact') {
    ...
    const existingRetry = store.all<Artifact>('artifacts').find(a => a.retryOf === failed.id && (a.status !== 'failed' || (command.requestId && a.requestId === command.requestId)));
    if (existingRetry) return existingRetry;
    artifact = {...failed,id:ctx.id(),retryOf:failed.id,attemptRoot:failed.attemptRoot ?? failed.id,requestId:command.requestId ?? ctx.id(),status:'producing',versions:[],lastError:undefined,createdAt:ctx.now().toISOString(),leaseOwner:null,leaseUntil:null};
    store.put('manual_retries',{id:ctx.id(),artifactId:failed.id,newArtifactId:artifact.id,reason:command.reason,createdAt:ctx.now().toISOString()});
  } else {
    ...
    if (existing) {
      if (existing.kind !== command.kind || existing.origin !== command.origin || (command.topic && existing.topic !== command.topic) || (command.ageSegment && existing.ageSegment !== command.ageSegment) || (command.sourceId && existing.sourceIds[0] !== command.sourceId) || (command.segments && JSON.stringify(existing.segments) !== JSON.stringify(intervals(command.segments)))) throw new Error('Request ID is already owned by different work');
  ...
  const freeBytes = ctx.adapters.storage?.freeBytes ? await ctx.adapters.storage.freeBytes() : filesystem.bavail * filesystem.bsize;
  ...
  const acquired = store.transaction(() => {
    const retryOwner = artifact.retryOf && store.all<Artifact>('artifacts').find(a=>a.retryOf === artifact.retryOf && a.id !== artifact.id && a.status !== 'failed');
    if (retryOwner) return retryOwner;
    const requestOwner = artifact.requestId && store.all<Artifact>('artifacts').find(a=>a.requestId === artifact.requestId && a.id !== artifact.id);
    if (requestOwner) {
      if (requestOwner.kind !== artifact.kind || requestOwner.origin !== artifact.origin || requestOwner.topic !== artifact.topic || requestOwner.ageSegment !== artifact.ageSegment || JSON.stringify(requestOwner.sourceIds) !== JSON.stringify(artifact.sourceIds) || JSON.stringify(requestOwner.segments) !== JSON.stringify(artifact.segments)) throw new Error('Request ID is already owned by different work');
      return requestOwner;
    }
    ...
    if (!claimWorkload(ctx, `production:${artifact.id}`)) return {status:'deferred',reason:'workload-limit'};
    if (artifact.kind === 'video') {
      assertPermission(source,ctx.now());
      artifact.segments = intervals(artifact.segments);
      artifact.sourceEvidence = {sourceId:source!.id,filePath:source!.filePath,integrity:source!.integrity,permission:structuredClone(source!.permission)};
      ...
      for (const segment of artifact.segments) {
        const overlap = store.all<RecordData>('segments').find(s=>s.sourceId === source!.id && s.artifactId !== artifact.id && s.attemptRoot !== (artifact.attemptRoot ?? artifact.id) && s.startMs < segment.endMs && segment.startMs < s.endMs);
        if (overlap && command.allowReuse !== true) throw new Error('Source interval already reserved or used; explicit reuse required');
      }
      for (const segment of artifact.segments) store.put('segments',{id:`${artifact.id}:${segment.startMs}:${segment.endMs}`,artifactId:artifact.id,attemptRoot:artifact.attemptRoot ?? artifact.id,sourceId:source!.id,...segment,status:'reserved',explicitReuse:command.allowReuse === true});
    }
...
async function produceArtifact(ctx: Context, artifact: Artifact, command: Command, source: RecordData|undefined, owner: string): Promise<Artifact> {
  ...
  const bounded = async (operation: (signal: AbortSignal) => Promise<any>) => {
    ...
    try {
      return await Promise.race([operation(controller.signal), new Promise((_, reject) => { timer = setTimeout(() => { controller.abort(); reject(new Error('Production task timed out; manual retry required')); }, ctx.config.limits.taskTimeoutMs); })]);
  ...
  try {
    ...
    while (artifact.versions.length < 3 || !artifact.versions.at(-1)?.review) {
      ...
      if (!version.filePath) {
        ...
        artifact.leaseUntil = new Date(ctx.now().getTime()+ctx.config.limits.leaseMs).toISOString();
        store.put('artifacts',artifact);
        const result = await bounded(signal => ctx.adapters.editor.create({signal,artifact,version:version.number,mission:ctx.mission,source,feedback:previous?.review?.findings ?? [],outputDirectory,idempotencyKey:version!.id}));
        owned();
    ...
    if (['RATE_LIMIT','QUOTA_EXCEEDED','rate_limit','rate-limit','quota'].includes(error.code ?? error.kind)) {
