...
export function createHarness(options:HarnessOptions={}) {
  ...
  const adapters={storage:{freeBytes:async()=>{const fs=statfsSync(config.storage.mediaDirectory);return fs.bavail*fs.bsize;}},...options.adapters};
  const ctx:Context={store,config,adapters,now:options.clock??(()=>new Date()),random:options.random??Math.random,id:randomUUID,
  ...
  function safeConfig() { const value=structuredClone(config); delete value.integrations; return value; }
  function snapshot() {
    return store.transaction(()=>{
      const date=today(ctx);
      const previous=store.get('plans',date);
      if(previous?.snapshotSealed) return previous;
      return store.put('plans',{...previous,id:date,date,config:safeConfig(),snapshotSealed:true,createdAt:ctx.now().toISOString()});
    });
  }
  async function dispatch(command:Command):Promise<any> {
    ...
    switch(command.type) {
      ...
      case 'doctor': case 'backup': {
        const {maintenance}=await import('./maintenance.ts');
        return maintenance(ctx,command);
      }
      case 'request': case 'tick': {
        const coordinator=await import('./coordinator.ts');
        return command.type==='request'?coordinator.request(ctx,command,execute):coordinator.tick(ctx,command,execute);
      }
  ...
  async function execute(command:Command):Promise<any> {
    ...
    if(typeof command.commandId!=='string'||command.commandId.length>200) throw new Error('Invalid command identity');
    const fingerprint=createHash('sha256').update(JSON.stringify(command)).digest('hex');
    const id=`command:${command.commandId}`;
    const existing=store.transaction(()=>{
      const previous=store.get('tasks',id);
      if(previous) {
        if(previous.fingerprint!==fingerprint) throw new Error('Identity belongs to a different command');
        return previous;
      }
      store.put('tasks',{id,fingerprint,status:'running',createdAt:ctx.now().toISOString()});
      return undefined;
    });
    if(existing) {
      if(existing.status==='complete') {
        if(command.commandId.startsWith('telegram:')) await ctx.notify({type:'command-result',commandId:command.commandId,result:existing.result});
        return existing.result;
      }
      if(existing.status==='failed') throw new Error(existing.error);
      return {status:'uncertain',commandId:command.commandId,message:'Command already accepted; inspect state before an explicit retry'};
    }
    try {
      const result=await dispatch({...command,requestId:command.requestId??command.commandId});
      store.put('tasks',{id,fingerprint,status:'complete',result:result??null});
      if(command.commandId.startsWith('telegram:')) await ctx.notify({type:'command-result',commandId:command.commandId,result});
      return result;
    } catch(error) {
