...
test("configuration accepts zero overrides, rejects malformed counts, and seals daily settings across restart", async () => {
  ...
  const config = {
    storage: { databasePath: join(root, "state.sqlite"), mediaDirectory: join(root, "media") },
    daily: { videos: 0, images: 0, texts: 0 },
  };
  let app = createHarness({
    config,
    env: { DAILY_VIDEO_COUNT: "0" },
  ...
  try {
    ...
    app.close();
    app = createHarness({
      config: { ...config, daily: { videos: 1, images: 1, texts: 2 } },
      env: {},
    ...
    assert.throws(
      () => createHarness({ config, env: { DAILY_VIDEO_COUNT: "1x" } }),
      /DAILY_VIDEO_COUNT/
    );
  } finally {
    app.close();
...
test("LLM configuration requires explicit task routes and rejects the retired global provider", async () => {
  ...
  assert.throws(
    () => loadConfig({ llm: { provider: "codex" } } as any, {}),
    /llm\.provider is no longer supported/
  );
  const config = loadConfig(
    { llm: { taskModels: { review: { provider: "codex", model: "review-model" } } } } as any,
    {}
  );
...
test("durable command identity returns one snapshot after restart and rejects changed payload", async () => {
  ...
  try {
    ...
    app.close();
    app = createHarness({ config, env: {} });
    assert.deepEqual(
      await app.execute({ type: "daily-snapshot", commandId: "telegram:123" }),
      first
    );
    await assert.rejects(
      app.execute({ type: "pause", paused: true, commandId: "telegram:123" }),
      /different command/
    );
  } finally {
    app.close();
...
test("bounded ticks keep filling future dates and custom requests remain additional work", async () => {
  ...
  writeFileSync(output, "আজ শিশুকে কাগজ দিয়ে নিজের কিছু বানাতে দিন।");
  const seen: string[] = [];
  const app = createHarness({
    env: {},
    config: {
    ...
    adapters: {
      editor: {
        create: async (input: any) => {
          seen.push(input.artifact.origin);
  ...
  try {
    ...
    const custom = await app.execute({
      type: "request",
      texts: 1,
      topic: "custom topic",
      requestId: "custom-once",
    });
...
test("backup retains a restorable command snapshot and doctor reports missing live setup", async () => {
  ...
  try {
    ...
    const restored = createHarness({
      config: {
        storage: {
          databasePath: join(root, "backup", "sfurti.sqlite"),
          mediaDirectory: join(root, "backup", "media"),
        },
      },
      env: {},
    });
    try {
      assert.deepEqual((await restored.execute({ type: "status" })).plans[0], sealed);
    } finally {
      restored.close();
    }
  } finally {
...
test("pending Telegram results drain after restart without regenerating work or duplicating delivery keys", async () => {
  ...
  try {
    ...
    app = createHarness({
      ...
      adapters: {
        delivery: {
          send: async (event: any, options: any) => {
...
test("Telegram and discovery outages do not stop publication of ready custom content", async () => {
  ...
  const { writeFileSync } = await import("node:fs");
  const output = join(root, "text.txt");
  writeFileSync(output, "আজ শিশুর সঙ্গে তৈরি করি।");
  let submitted = 0;
  const app = createHarness({
    env: {},
    config: {
      storage: { databasePath: join(root, "db"), mediaDirectory: join(root, "media") },
      daily: { videos: 1, images: 0, texts: 0 },
      posting: {
        windows: [{ start: "09:00", end: "20:00" }],
        minSpacingMinutes: 60,
        queueCsvPath: join(root, "queue.csv"),
    ...
    adapters: {
      ...
      editor: { create: async () => ({ filePath: output, caption: "তৈরি করি" }) },
      media: { inspect: async () => ({ valid: true, evidence: {} }) },
      reviewer: {
        review: async () => ({
          passed: true,
      ...
      facebook: {
        ...
        submit: async () => {
          submitted++;
          return { status: "scheduled", remoteId: "remote" };
        },
