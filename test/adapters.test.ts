...
test("external process treats user input as data and returns Unicode intact", async () => {
  const adapter = new ProcessAdapter({
    ...
    args: [
      "-e",
      `let s='';process.stdin.on('data',c=>s+=c);process.stdin.on('end',()=>process.stdout.write(JSON.stringify({protocol:'sfurti/1',ok:true,result:JSON.parse(s).payload})))`,
    ],
  });
  assert.deepEqual(await adapter.call("render", { text: "বাংলা; $(exit 1)" }), {
    text: "বাংলা; $(exit 1)",
  });
});
test("external process timeout has an uncertain outcome and bounded output is enforced", async () => {
  ...
  await assert.rejects(
    stalled.call("submit", {}),
    (e: unknown) => e instanceof AdapterError && e.kind === "timeout" && e.uncertain
  );
  const noisy = new ProcessAdapter({
    executable: process.execPath,
  ...
  await assert.rejects(
    noisy.call("submit", {}),
    (e: unknown) => e instanceof AdapterError && e.kind === "output_limit" && e.uncertain
  );
});
test("Telegram rejects group and foreign operator requests before application execution", async () => {
  const commands: unknown[] = [];
  const operator = new TelegramOperator({
    operatorUserId: "42",
    app: {
  ...
  assert.equal(
    await operator.dispatch({
      update_id: 100,
      message: { from: { id: 42 }, chat: { id: -1, type: "group" }, text: '{"type":"status"}' },
    }),
    undefined
  );
  assert.equal(
    await operator.dispatch({
      update_id: 100,
      message: { from: { id: 7 }, chat: { id: 7, type: "private" }, text: '{"type":"status"}' },
    }),
    undefined
  );
  ...
  assert.equal(
    await operator.dispatch({
      update_id: 100,
      message: { from: { id: 42 }, chat: { id: 42, type: "private" }, text: '{"type":"status"}' },
    }),
    "done"
  );
...
test("split UTF-8 bytes survive the process protocol", async () => {
  const adapter = new ProcessAdapter({
    ...
    args: [
      "-e",
      `const b=Buffer.from(JSON.stringify({protocol:'sfurti/1',ok:true,result:'বাংলা'}));let i=0;const timer=setInterval(()=>{process.stdout.write(b.subarray(i,i+1));if(++i===b.length)clearInterval(timer)},1)`,
    ],
  });
...
test("Facebook malformed replies remain uncertain instead of inventing confirmed publication", async () => {
  ...
  const adapters = createConfiguredAdapters({
    facebook: {
      ...
      args: [
        "-e",
        `process.stdout.write(JSON.stringify({protocol:'sfurti/1',ok:true,result:{status:'scheduled'}}))`,
      ],
    },
  });
  await assert.rejects(
    adapters.facebook!.submit({ id: "stable-1" }),
    (e: unknown) => e instanceof AdapterError && e.kind === "protocol" && e.uncertain
  );
...
test("Codex decisions use independent restricted contexts and distinguish tokens from unknown capacity", async () => {
  ...
  try {
    const strategy = new CodexStrategy(
      { authFile: join(root, "auth.json"), models: ["configured-model"] },
      (options) => {
        assert.equal(
          options.config?.features &&
            typeof options.config.features === "object" &&
            !Array.isArray(options.config.features)
            ? options.config.features.shell_tool
            : undefined,
          false
        );
    ...
    const request = {
      ...
      schema: { type: "object" },
      validate: (v: unknown): v is { passed: boolean } =>
        typeof v === "object" && v !== null && "passed" in v && typeof v.passed === "boolean",
    };
    ...
    await assert.rejects(
      strategy.structured("configured-model", { ...request, requiredCapabilities: ["video"] }),
      (e: unknown) => e instanceof AdapterError && e.kind === "rejected"
    );
  } finally {
...
test("LLM task routes dispatch configured models and reject missing, unknown, and unsupported routes", async () => {
  ...
  const router = new TaskModelRouter(
    {
      generation: { provider: "fake", model: "small" },
      review: { provider: "fake", model: "strong" },
    },
    { fake: provider }
  );
  const request = {
  ...
  assert.deepEqual(calls, [
    { model: "small", purpose: "generation" },
    { model: "strong", purpose: "review" },
  ]);
  assert.throws(
    () => new TaskModelRouter({ review: { provider: "missing", model: "x" } }, { fake: provider }),
    (e: unknown) => e instanceof AdapterError && /unknown provider/.test(e.message)
  );
  assert.throws(
    () =>
      new TaskModelRouter({ review: { provider: "fake", model: "missing" } }, { fake: provider }),
    (e: unknown) => e instanceof AdapterError && /unsupported/.test(e.message)
  );
  await assert.rejects(
    new TaskModelRouter({}, { fake: provider }).structured("review", {
      ...request,
      purpose: "review",
    }),
    (e: unknown) => e instanceof AdapterError && /no configured/.test(e.message)
  );
  await assert.rejects(
    router.structured("review", { ...request, purpose: "review", requiredCapabilities: ["video"] }),
    (e: unknown) => e instanceof AdapterError && /does not support video/.test(e.message)
  );
});
...
test("an external reviewer retains precedence and the Codex reviewer requires its review route", async () => {
  ...
  await assert.rejects(
    adapters.reviewer!.review({ artifact: {}, version: {}, source: {}, evidence: {} }),
    (e: unknown) => e instanceof AdapterError && /review has no configured/.test(e.message)
  );
});
