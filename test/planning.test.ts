...
async function fixture(facebook: any = {}, override: any = {}) {
  ...
  const output = join(root, "text.txt");
  await writeFile(output, "শিশুকে নিজের হাতে কিছু তৈরি করতে দিন।");
  ...
  const adapters = {
    editor: {
      create: async () => ({ filePath: output, caption: 'একসঙ্গে, তৈরি করি\n"নতুন কিছু"' }),
    },
    media: {
      inspect: async () => ({
        valid: true,
  ...
  return {
    ...
    produce: async (origin = "reserve", requestId = crypto.randomUUID()) =>
      app.execute({ type: "produce", kind: "text", origin, requestId }),
  };
}
...
test("90 complete dates require 90 approved artifacts and retain random times after restart", async () => {
  ...
  try {
    ...
    try {
      await resumed.execute({ type: "plan" });
      assert.deepEqual((await resumed.execute({ type: "status" })).posts, before.posts);
    } finally {
      resumed.close();
    }
  } catch (error) {
...
test("a remote success followed by timeout reconciles before another creation and preserves history", async () => {
  ...
  const f = await fixture({
    bounds: async () => ({ minLeadMinutes: 10, maxLeadDays: 7 }),
    submit: async () => {
      submissions++;
      remote = true;
    ...
    reconcile: async () =>
      remote ? { status: "scheduled", remoteId: "remote-1" } : { status: "unknown" },
...
test("unresolved submissions block rebuild and uncertain cancellations never resubmit", async () => {
  ...
  try {
    await f.produce();
    await f.app.execute({ type: "plan", startDate: "2026-09-07", days: 1 });
    await f.app.execute({ type: "publish" });
...
test("impossible posting quota fails even with an empty library", async () => {
  const f = await fixture(
    {},
    {
      daily: { videos: 0, images: 0, texts: 2 },
      posting: {
        windows: [{ start: "09:00", end: "09:20" }],
        minSpacingMinutes: 60,
        queueCsvPath: join(tmpdir(), crypto.randomUUID() + ".csv"),
      },
    }
  );
  try {
    await assert.rejects(f.app.execute({ type: "plan", days: 1 }), /spacing conflict/);
  } finally {
    f.app.close();
  }
});
...
test("distant local plans submit only inside platform bounds, then publication is reconciled", async () => {
  ...
  const f = await fixture({
    ...
    submit: async () => {
      submits++;
      return { status: "scheduled", remoteId: "r" };
    },
    reconcile: async () => ({
      status: published ? "published" : "scheduled",
      remoteId: "r",
      publishedAt: published ? "2026-10-06T04:00:00Z" : undefined,
    }),
  });
  try {
    await f.produce();
    await f.app.execute({ type: "plan", startDate: "2026-10-06", days: 1 });
    await f.app.execute({ type: "publish" });
    ...
    assert.equal(submits, 1);
    published = true;
    await f.app.execute({ type: "reconcile" });
    assert.equal((await f.app.execute({ type: "status" })).posts[0].status, "published");
  } finally {
    f.app.close();
  }
});
...
test("publication lease blocks a second coordinator while a remote submission is in flight", async () => {
  ...
  try {
    await f.produce();
    await f.app.execute({ type: "plan", days: 1 });
    const first = f.app.execute({ type: "publish" });
    await entered;
    ...
    release({ status: "scheduled", remoteId: "r" });
    await first;
    assert.equal(submits, 1);
  } finally {
    second.close();
...
test("configuration changes rebuild future local rows, preserve confirmed schedules and sealed today, and omit integration secrets", async () => {
  const f = await fixture(
    {
      bounds: async () => ({ minLeadMinutes: 10, maxLeadDays: 1 }),
      submit: async () => ({ status: "scheduled", remoteId: "r" }),
      reconcile: async () => ({ status: "scheduled", remoteId: "r" }),
    },
    { integrations: { secret: "secret-value" } }
  );
  try {
    await f.app.execute({ type: "daily-snapshot" });
    for (let i = 0; i < 3; i++) await f.produce();
    await f.app.execute({ type: "plan", days: 3 });
    await f.app.execute({ type: "publish" });
    const before = await f.app.execute({ type: "status" });
    assert.equal(before.posts[0].status, "scheduled");
    const changed = createHarness({
      config: { ...f.config, topic: "new-topic" },
      adapters: f.adapters,
      clock: f.clock,
      env: {},
    });
...
test("changed artifact bytes block publication after planning", async () => {
  ...
  try {
    const artifact = await f.produce();
    await f.app.execute({ type: "plan", days: 1 });
    await writeFile(artifact.filePath, "changed");
    await f.app.execute({ type: "publish" });
    assert.equal(submits, 0);
  ...
  const f = await fixture(
    {
      ...
      submit: async () => {
        submissions++;
        return { status: "scheduled", remoteId: `r${submissions}` };
      },
    },
    { daily: { videos: 2, images: 0, texts: 0 } }
  );
  try {
    const video = join(f.root, "video.mp4"),
      frame = join(f.root, "frame.png"),
      permission = join(f.root, "permission.md");
    await writeFile(video, "controlled video fixture");
    await writeFile(frame, "controlled frame fixture");
    await writeFile(permission, "Editing and Facebook use authorized");
    (f.adapters.editor as any).create = async () => ({
      filePath: video,
      caption: "কাগজ দিয়ে তৈরি করি",
    });
    (f.adapters.media as any).inspect = async () => ({
      valid: true,
      width: 1080,
      height: 1920,
      durationSeconds: 35,
      evidence: {
        ...
        audio: { intelligible: true, coverage: "Full audio" },
        coverage: "Sampled frame with transcript",
        limitations: "Visual inspection sampled",
      },
    });
    ...
    for (const [sourceId, startMs] of [
      ...
      const result = await f.app.execute({
        ...
        origin: "reserve",
        sourceId,
        segments: [{ startMs, endMs: startMs + 35000 }],
      });
      assert.equal(result.status, "approved");
    }
    const coverage = await f.app.execute({ type: "plan", days: 2 });
    assert.equal(coverage.days[0].complete, true);
    assert.equal(coverage.days[1].missing.video, 1);
    const state = await f.app.execute({ type: "status" }),
      selected = state.posts.filter((p: any) => p.date === "2026-09-06");
    assert.equal(
      new Set(
        selected.map(
          (p: any) => state.artifacts.find((a: any) => a.id === p.artifactId).sourceIds[0]
        )
      ).size,
      2
    );
    assert.ok(
      Math.abs(Date.parse(selected[0].scheduledAt) - Date.parse(selected[1].scheduledAt)) >= 3600000
    );
    await f.app.execute({
      type: "register-source",
      language: "bn",
    ...
    assert.equal(submissions, 1);
    assert.equal(
      (await f.app.execute({ type: "status" })).posts.filter((p: any) => p.status === "failed")
        .length,
      2
    );
  } finally {
