...
Each LLM task has an explicit route under `llm.taskModels`; the current task names are `generation` and `review`. A route is `{"provider":"codex","model":"MODEL_ID"}`. `integrations.codex` supplies authentication through either `apiKeyEnv` or `authFile`, plus optional positive `timeoutMs`, `maxOutputBytes`, and a `models` allowlist. When an allowlist is configured, every assigned Codex model must appear in it. Credentials are not embedded in prompts. A configured external reviewer takes precedence; otherwise the configured `review` route supplies the independent reviewer. Editors remain external processes; the strategy can also serve provider-neutral structured generation decisions through its public interface.

Move an existing single-model setting such as `integrations.codex.model: "MODEL_ID"` to every task assignment that should use it, for example `llm.taskModels.review.model: "MODEL_ID"`; then remove `integrations.codex.model`. The former `llm.provider` setting is rejected so routes cannot change silently. Configure a route only for task types the deployment uses; a missing route fails that LLM task clearly. There are no per-call model overrides or fallback routes.
